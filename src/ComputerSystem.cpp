...
void* ComputerSystem::MapDisplay() {
	...
	// Repeat infinitely with a period of 5seconds
	while (true) {
		vector<AircraftData> aircraftsInfo;
		Radar radar;

		// Initialize thread vector for thread deletion at the end of current period
...
void ComputerSystem::computerSystemOperations() {
	...
	while (true) {
		...
		if (aircraftCommand.header.type == 0x02 && aircraftCommand.header.subtype == 0x00) {
			...
			// 3. Retrieve aircraft based on aircraftCommand.flightId (received from Operator)
